...
module.exports = (req, res, next) => {
  if(!req.headers.authorization) {
    return next(createError(400, 'no auth header')); //create error handling middleware to mount in server/app.use [if err.status exists, res.send and next it]
  }
  ...
  jwt.verify(token, process.env.SECRET || 'DEV', (err, decodedToken) => {
    ...
    .catch(() => next(createError(404, 'user not fucking found')));
  });
};
